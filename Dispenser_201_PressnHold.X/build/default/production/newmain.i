...
void main(void) {
    ...
    while (1) {
        while (DispenserOn) {
            ...
            uint32_t counter = 30 * ((200 * ((uint32_t) AdcReadResult())) / 1023);
            DioPinWrite(1, 1);
            uint8_t DisplayUpdate = 1;
            for (; (counter > 0) && (!(GPIO & (1 << 5))); counter--) {
                _delay((unsigned long)((4600)*(4000000/4000000.0)));
            }
            DispenserOn = 0;
            DioPinWrite(1, 0);
        }
...
void __attribute__((picinterrupt(("")))) ISR() {
    ...
    for(uint8_t DebounceCounter = 0; (!(GPIO & (1 << 5))); DebounceCounter++) {
