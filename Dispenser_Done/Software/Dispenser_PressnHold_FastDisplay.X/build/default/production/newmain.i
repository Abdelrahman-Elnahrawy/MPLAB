        ...
        while (DispenserOn) {
            ...
            for (;(Counter > 0) && (!(GPIO & (1 << 3))); Counter--) {
                if (Counter % 10 == 0) {
                    ...
                    for (uint16_t i = 1; i < 10000; i *= 10, Current_Digit++) {
                        uint16_t value = (Counter % (i * 20)) / (i*2);
                        if (i == 100) {
                            SpiSend16bit(((uint16_t) Current_Digit) << 8 | (0x80 | MAX7219_No_Table[value]));
                        } else {
                            SpiSend16bit(((uint16_t) Current_Digit) << 8 | (MAX7219_No_Table[value]));
                        }
                    }
                }
                                _delay((unsigned long)((1250)*(4000000/4000000.0)));
            }
            DispenserOn = 0;
            DioPinWrite(4, 0);
