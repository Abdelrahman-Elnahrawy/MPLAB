...
void main(void) {
    ...
    while (1) {
        while (DispenserOn) { //Check if the flag of Dispenser operation is set
            ...
            for (; (counter > 0) && (!DIO_READ_PIN(GPIO_PIN_3)); counter--) {
                if (counter % 100 == 0 || DisplayUpdate) {   
                    // Update display every 100 ticks (~5 ms) 
                    uint16_t CountDownSeconds = (uint16_t)(counter / 200); 
                    MAX7219_DISPLAYDATA(MAX7219_REG_DIGIT3, MAX7219_No_Table[CountDownSeconds / 600]); // Minutes (ten)
                    MAX7219_DISPLAYDATA(MAX7219_REG_DIGIT2, MAX7219_DOT_BIT | MAX7219_No_Table[(CountDownSeconds / 60) % 10]); // Minutes (unit)
                    MAX7219_DISPLAYDATA(MAX7219_REG_DIGIT1, MAX7219_No_Table[(CountDownSeconds % 60) / 10]); // Seconds (ten)
                    MAX7219_DISPLAYDATA(MAX7219_REG_DIGIT0, MAX7219_No_Table[(CountDownSeconds % 60) % 10]); // Seconds (unit)
                    DisplayUpdate = 0;
                }
                __delay_us(3645); // Delay adjusted for timing (the delay value is calculated manually via try and error ) 
            }
...
void __interrupt() ISR() {  // interrupt on change on pin3 as its the only pin that has interrupt on change enabled
    ...
    for(uint8_t DebounceCounter = 0; (!DIO_READ_PIN(GPIO_PIN_3)); DebounceCounter++) { 
        __delay_ms(1); 
