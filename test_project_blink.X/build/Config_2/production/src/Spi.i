...
# 4 "src/../config/Spi_Cfg.h" 2

# 1 "src/../config/../config/Spi_Cfg.h" 1
# 5 "src/../config/../config/Spi_Cfg.h" 2

# 1 "src/../config/../include/Spi_Types.h" 1
...
# 6 "src/../config/../config/Spi_Cfg.h" 2



typedef struct {
    Spi_ModeType spiMode;
    Spi_ClockSpeedType clockSpeed;
    uint8 sckPin;
    uint8 mosiPin;
    uint8 misoPin;
    uint8 ssPin;
...
static void Spi_Delay(Spi_ClockSpeedType clockSpeed) {
    switch (clockSpeed) {
        ...
        case SPI_CLOCK_DIV64:
            _delay((unsigned long)((16)*(4000000/4000000.0)));
            break;
...
uint8 Spi_TransmitReceive(uint8 data) {
    ...
    for (uint8 i = 0; i < 8; i++) {
        ...
        if (data & 0x80) {
            GPIO |= (1 << Spi_Configurations.mosiPin);
        } else {
            GPIO &= ~(1 << Spi_Configurations.mosiPin);
        }
        ...
        if (Spi_Configurations.spiMode == SPI_MODE0 || Spi_Configurations.spiMode == SPI_MODE2) {
            ...
            GPIO &= ~(1 << Spi_Configurations.sckPin);
...
uint8 Spi_Receive(void) {
    return Spi_TransmitReceive(0xFF);
}
