...
void main(void) {
    InitAll();
    while(1){
        if (DispenserOn &&( DebounceCounter > ((100)/(10)) )){
                DioPinWrite(1, 1);
                uint32_t Counter = 5 * ((200 * ((uint32_t) PotInValue )) / 1023);
...
T1CON &= ~0b00000001 ; TMR1 = (65535 - 1250); T1CON |= 0b00000001 ;;
# 205 "main.c"
PIE1 = 0b01000001;
}
...
void Timer1_interrupt (){
    ...
    if(LastSensorInValue > 256 && SensorInValue < 256 ){
        NegativeEdge();
    }

    if(DispenserOn && SensorInValue < 256 ){
        DebounceCounter++;
    } else {
        DebounceCounter = 0;
    }
    if( SensorInValue > 256 ){
        DispenserOn = 0;
    }
}
